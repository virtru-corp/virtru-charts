...
| `bootstrap.configFile`                              | The configuration file content (set using --set-file bootstrap.configPath=yourconfig.yaml | `nil`                                         |
...
| `entitlement-policy-bootstrap.policyDataSecretRef`  | Secret name used to mount policy artifacts into the job. Example: kubectl create secret generic my-secret --from-file=pathTo/entitlement-policy | `scp-bootstrap-entitlement-policy`               |
